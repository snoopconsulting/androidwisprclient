		...
		if (isConnectedIntent(intent)) {
			...
			if (FONUtil.isSupportedNetwork(ssid, bssid)) {
				...
				if (active) { //SI el autologin
					String username = getPreferences(context).getString(context.getString(R.string.pref_username), "fabiosimonagio@gmail.com@wisprlinktelwifi");
					String password = getPreferences(context).getString(context.getString(R.string.pref_password), "fabiosimonagio");
